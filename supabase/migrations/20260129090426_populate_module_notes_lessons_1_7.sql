/*
  # Populate Module Notes with Lesson Content (Lessons 1-7)

  This migration inserts comprehensive revision notes for all 7 modules
  based on the detailed PDF content provided. Each module contains:
  - Main Ideas section
...
(4, 'Module 4: When to Use AI (and When Not To)', jsonb_build_array(
  jsonb_build_object(
    'name', 'Main Ideas',
    'subsections', jsonb_build_array(
      jsonb_build_object('title', 'AI as Superpower or Downfall', 'content', 'AI is a double-edged sword: it can make you 10× more powerful, or make you weak and dependent if you use it badly. Like calculators in primary school or eating only bananas, using AI too early or for everything can damage your basic skills and mental "diet". The key motto is: "Use AI – don''t let AI use you."'),
...
(7, 'Module 7: Your Future Strategy in the AI Era', jsonb_build_array(
  jsonb_build_object(
    ...
    'subsections', jsonb_build_array(
      ...
      jsonb_build_object('title', 'Strategic Mistakes to Avoid', 'content', 'AI-dependency trap: moving from "AI helps me" to "I can''t do this without AI", which weakens your own abilities. Information ≠ knowledge: AI gives instant facts, but real knowledge needs struggle, making connections, applying ideas and testing your understanding. Identity trap: building your identity on how you work (typing speed, memorising facts, old methods) instead of what you deliver (problem-solving, clear communication, building useful things).'),
