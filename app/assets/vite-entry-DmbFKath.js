...
</div>`,activity:"<p>Activity content coming soon</p>"},getLesson(e,t){const n=`${e}-${t}`;return this[n]}},L={bg:l.colors.background.light||l.colors.background.base,cardBg:l.colors.background.card,textPrimary:l.colors.text.primary,textSecondary:l.colors.text.secondary,textMuted:l.colors.text.muted,accent:l.colors.accent.cyan,accentLight:`${l.colors.accent.cyan}15`,primary:l.colors.primary.deep,border:l.colors.border.subtle,success:l.colors.status.success};function Yy(){const{user:e,signOut:t}=Je();Nt();const{moduleId:n,lessonId:o}=ci(),[s,r]=b.useState(!0),[a,c]=b.useState(!1),[u,d]=b.useState(!1),[m,h]=b.useState(!1),[g,p]=b.useState(!1),[v,x]=b.useState(null),{isComplete:A,toggleComplete:y,loading:f}=mi(e==null?void 0:e.id,[]),{isBookmarked:w,toggleBookmark:k}=$m(e==null?void 0:e.id),S=parseInt(n),T=parseInt(o),N=Se.find(pe=>pe.id===S),R=N==null?void 0:N.lessons.find(pe=>pe.id===T);if(!N||!R)return i.jsx("div",{className:"flex-1 p-8",children:i.jsx("p",{children:"Lesson not found"})});if(f)return i.jsx("div",{className:"flex-1 flex items-center justify-center",children:i.jsx("div",{className:"text-gray-600",children:"Loading..."})});const C=Hy.getLesson(S,T),I=`${S}-${T}`,M=ni[I],_=dn(S,T),H=A(_,"lesson");if(!C)return i.jsx("div",{className:"flex-1 p-8",children:i.jsx("p",{children:"Lesson content not available"})});const ke=pe=>{const z=document.createElement("div");z.innerHTML=pe;let E="";const B=(U,ae="")=>{if(U.nodeType===Node.TEXT_NODE){const Ae=U.textContent.trim();Ae&&(E+=Ae+" ")}else if(U.nodeType===Node.ELEMENT_NODE){const Ae=U.tagName.toLowerCase();Ae==="h3"||Ae==="h2"?(E.trim()&&(E=E.trim()+`
